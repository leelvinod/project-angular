...
export class AgentelecomService {

  baseUrl: string;

  constructor(private http: Http) {
    this.baseUrl = "http://localhost:8084/csa"
  }
  ...
  getBaseUrlForGetByLocation(location): string {
    return this.baseUrl + "/getAll/" + location
  }
  getByLocation(location: string): Observable<Customer> {
    return this.http.get(this.getBaseUrlForGetByLocation(location)).pipe(map(data => data.json()));
  }
  ...
  updateCustomer(customer: Customer) {
    return this.http.put(this.getBaseUrlForUpdateTicket(), JSON.stringify(customer), this.getJsonContentTypeHeader());
  }
